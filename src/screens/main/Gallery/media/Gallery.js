...
function Gallery({ navigation }) {
    ...
    useEffect(() => {
        navigation.setOptions({
            headerRight: () => (
                <TouchableOpacity 
                    onPress={() => onNavigate('mediaPost', selectedId)}
                >
                    <Text style={{fontSize: 17, marginRight : 10}}>Next</Text>
                </TouchableOpacity>
            ),
            headerLeft: () => (
    ...
    function renderItem({ item }) {
        return (
            <TouchableOpacity style={{ flex: 1, width: 100, padding: 3}} onPress={() => selectImage(item['id'])}>
